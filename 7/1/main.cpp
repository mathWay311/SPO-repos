...
int main()
{
    ...
    signal(SIGALRM, terminated_by_alarm); // Привязка сигнала к срабатыванию таймера.
    ...
    signal(SIGUSR1, recieve_message);                   // Установка по срабатыванию SIGUSR1 - вызвать recieve_message
    if(fork() == 0)
    {
